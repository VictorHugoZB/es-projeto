...
export default function AccountsProvider({ children }) {
  ...
  const login = (email, pass) => {
    const filteredUsers = accounts.users.filter((u) => u.email === email && u.senha === pass);
    if (filteredUsers.length) return 'user';

    const filteredSpecialists = accounts.specialists
      .filter((u) => u.email === email && u.senha === pass);
    if (filteredSpecialists.length) return 'specialist';

    if (accounts.adms[0].email === email && accounts.adms[0].senha === pass) return 'adm';
